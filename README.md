...
**Phase 0(環境セットアップ + 最初の Domain)完了** / Phase 1(Domain 拡張)着手前

| 項目 | 値 |
| ---- | ---- |
| マージ済 PR | 8 |
| NUnit テスト | 25 件全緑(0.066 秒) |
| Domain C0 カバレッジ | 100%(83/83 行、18/18 メソッド) |
| EARS 要件 ID | 29(CARD 8 / PILE 13 / RND 5 / CFG 3) |
...
| 主対象プラットフォーム | WebGL |
| 副対象プラットフォーム | StandaloneOSX(Phase 1 以降で再評価) |
...
Assets/_Project/Scripts/
  Domain/
    Cards/          CardId / Pile
    Configuration/  IGameConfig (Phase 0 ではひな形)
    Random/         IRandomSource / XorShiftRandom
    Drowsy.Domain.asmdef  (noEngineReferences: true)
  Application/      Drowsy.Application.asmdef        (Phase 1 以降で実装)
  Infrastructure/   Drowsy.Infrastructure.asmdef     (Phase 1 以降で実装)
  Presentation/     Drowsy.Presentation.asmdef       (Phase 1 以降で実装)
  Bootstrap/        Drowsy.Bootstrap.asmdef          (Phase 1 以降で実装)
  Tests/Domain.Tests/
    Cards/          CardIdTests / PileTests
...
| [`uv`](https://github.com/astral-sh/uv) | 任意 | unity-mcp 連携時の Python 実行(Phase 1 以降想定) |
...
| `git add` → pre-commit | commit 直前 | lefthook 5 commands |
...
| `spec-files` | Domain `.cs` 追加時に対応する EARS 仕様の存在確認 |
...
- [`CLAUDE.md`](CLAUDE.md) — プロジェクト規約(全 10 章)
