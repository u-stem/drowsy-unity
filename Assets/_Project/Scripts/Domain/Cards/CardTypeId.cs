...
{
    /// <summary>
    /// カードの種別(catalog の lookup key)を表す不変値オブジェクト(ADR-0018)。
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// M5 UI 実機検証で Hand 重複検出エラーが発覚し、ADR-0018 で本型を新設して分離した。
    /// </para>
    /// </remarks>
    public sealed record CardTypeId
    {
        ...
        /// の <c>"<typeId>#<instance>"</c> 区切り文字として ADR-0018 §8 で予約済)
        /// </exception>
        public static CardTypeId Of(string value)
        {
            ...
            if (value.IndexOf('#') >= 0)
            {
                // CardId.Value("<typeId>#<instance>")の区切り文字 '#' は ADR-0018 §8 で予約済。
