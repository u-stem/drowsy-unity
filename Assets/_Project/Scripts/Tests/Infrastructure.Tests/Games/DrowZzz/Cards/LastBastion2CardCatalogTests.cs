...
{
    /// <summary>
    /// カード No.14「最後の砦Ⅱ」(2026-05-17)の SO 表現と InMemory 表現の同値性検証(INF-157)。
    /// </summary>
    [TestFixture]
    public sealed class LastBastion2CardCatalogTests
    {
                ...
                effects: new EffectAsset[]
                {
                    new ChoiceEffectAsset(new[]
                    {
                        ...
                        new EffectBranchAsset(new EffectAsset[]
                        {
                            ...
                            new AssociateSpecificCardEffectAsset("15"),
                        }),
                    }),
                });
        ...
        [Test, Category("Small"), Category("Normal"), Property("Requirement", "INF-157")]
        public void Given_No14_SO_When_GetName_Then_InMemoryと一致()
        {
            Assert.That(NewSoCatalog().Get(CardTypeId.Of("14")).Name, Is.EqualTo(NewInMemoryCatalog().Get(CardTypeId.Of("14")).Name));
        }

        [Test, Category("Small"), Category("Normal"), Property("Requirement", "INF-157")]
        public void Given_No14_SO_When_GetEffects_Then_InMemoryとIEffect配列が記録値同値()
        {
            Assert.That(NewSoCatalog().GetEffects(CardTypeId.Of("14")), Is.EqualTo(NewInMemoryCatalog().GetEffects(CardTypeId.Of("14"))));
        }
    }
}
