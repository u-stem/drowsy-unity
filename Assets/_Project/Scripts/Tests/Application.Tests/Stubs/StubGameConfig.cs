...
namespace Drowsy.Application.Tests.Stubs
{
    ...
    /// </summary>
    /// <remarks>
    /// 本物の <c>ScriptableObject</c> ベース実装は <c>Drowsy.Infrastructure</c> 配下に存在する。
    /// <see cref="IGameConfig.DdpPool"/> 追加に追随して 2 引数 ctor を新設(後方互換維持のため
    /// 既存 1 引数 ctor は DdpPool デフォルト使用)。
    /// </remarks>
