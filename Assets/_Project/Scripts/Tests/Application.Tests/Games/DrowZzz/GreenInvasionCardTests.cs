...
namespace Drowsy.Application.Tests.Games.DrowZzz
{
    ...
    public sealed class GreenInvasionCardTests
    {
        ...
        public void Given_p2に影響カウント3_p1ターン終了_When_EndTurn_Then_p2のInfluenceカウントは不変3()
        {
            ...
            // When
            var next = rule.Apply(session, new EndTurnAction());
            // Then(p2 Tick で TickEffect 適用のみ、count は p2 自身の EndTurn で -1 されるまで 3 のまま)
            Assert.That(next.Influences[PlayerId.Of("p2")][0].RemainingCount, Is.EqualTo(3));
        }

        // ===== DZ-177: カウント 1 でも次の自フェーズで 1 回機能、自フェーズ終了で除去 =====
        ...
        {
            // Given(カウント 1 でも p2 の Tick で TickEffect が 1 回機能、count は p2 自身の EndTurn まで残存)
            var rule = NewRule(NewCatalogWithCardTwo());
        ...
        // ===== DZ-178: 他プレイヤーの影響は p1 EndTurn では変更されない(p2 Tick は count 不変) =====
        ...
        {
            // Given(p1 current、p1 と p2 双方が影響保有。p1 EndTurn 冒頭で p1 Decrement、Turn.Next 後 p2 Tick)
            var rule = NewRule(NewCatalogWithCardTwo());
        ...
        // ===== DZ-179: 自プレイヤーの影響は EndTurn 冒頭で count -1 される =====
        ...
        {
            // Given(p1 EndTurn 冒頭で p1 自身の Influences すべてに対して count -1)
            var rule = NewRule(NewCatalogWithCardTwo());
            var p1Inf = new PlayerInfluence(InfluenceTrigger.OwnPhaseStart, new AdjustSdpEffect(SdpTarget.Self, -1), 3);
