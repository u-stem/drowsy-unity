...
{
    /// <summary>
    /// 連想後使用制限マーカー効果(ADR-0011 §6 / M3-PR6 で新設)。**2 役を兼ねるマーカー**:
    /// <list type="number">
    ...
    /// <see cref="EffectInterpreter.Apply"/> で no-op(<c>RemainingCount</c> 1→0 で除去、<see cref="DrowZzzRule.TickInfluences"/> 経路)。</item>
    /// </list>
    /// </summary>
    /// <remarks>
    /// ADR-0011 §6 で確定した「次の自分のターン以降に限り使用可能」semantics を <see cref="Influences.PlayerInfluence"/> 流用
    /// (候補 C、JIT 確定 2026-05-14)で表現する。連想時に 1 フェーズ分の使用制限を付与し、次の自フェーズ開始時の
    ...
    /// 対象カードの効果列を **最上位 scan** する(`RequiresMinimumTotalPointsMarkerEffect` と同じスコープ)。
    /// </para>
    /// <para>
