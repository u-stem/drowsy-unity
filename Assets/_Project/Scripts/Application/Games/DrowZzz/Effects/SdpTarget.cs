namespace Drowsy.Application.Games.DrowZzz.Effects
{
    ...
    /// 「コップ一杯の脅威」が自分 / 相手の両方を変動させる必要があるため)。設計選択:
    /// <list type="bullet">
    ...
    /// </list>
    /// 「自分」「相手」(M2-PR5 で確立した表記、ボードゲーム自然語)を、コード identifier として英語の
    /// <c>Self</c> / <c>Opponent</c> に統一する(CLAUDE.md §1 識別子英語規約、ADR-0006 §1.1 と整合)。
    /// 過去 PR(M2-PR3)では「使用者 / 被使用者」、プロジェクトオーナー JIT 共有原文では「甲 / 乙」表記を
    /// 用いていたが、M2-PR5 でカード本文 / 仕様 / コメントは「自分 / 相手」に統一(green-invasion.md / cup-of-threat.md)。
    /// <para>
    ...
    public enum SdpTarget
    {
        ...
        /// <summary>相手(N=2 で現プレイヤー以外、カードをプレイされた側)。N>2 拡張は Phase 3 候補(ADR-0006 §Negative)。</summary>
        Opponent,
    }
}
