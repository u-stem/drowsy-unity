namespace Drowsy.Application.Games.DrowZzz.Effects
{
    ...
    /// <remarks>
    /// 「コップ一杯の脅威」(No.01)カードで自分 / 相手の両方の SDP を変動させる必要があるため、
    /// `SdpTarget` enum を positional 引数として効果 record 自身が保持する設計を採用。
    /// <para>
    /// 評価は <see cref="EffectInterpreter.Apply"/> で行う。0 floor は適用しない(「持ち点低い方が勝ち」と整合)。
    /// </para>
    /// </remarks>
