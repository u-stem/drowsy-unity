...
namespace Drowsy.Application.Games.DrowZzz.Effects
{
    ...
    /// <remarks>
    /// <para>
    /// 既存 <see cref="Drowsy.Application.Games.DrowZzz.AssociateAction"/> が
    /// 「プレイヤーの能動アクションによる連想」(TotalPoints>=80 + AssociatableMarkerEffect 必須)だったのに対し、
    /// 本 effect は **カード効果として自動発動する連想** で、合法性条件(threshold / marker)は課さない。
    /// </para>
    /// <para>
    ...
    {
        // null 防御の二重ガード(positional ctor 経由 + init setter 経由)
