namespace Drowsy.Application.Games.DrowZzz
{
    ...
    /// </summary>
    /// <remarks>
    /// マジックナンバー禁止方針に従い、<see cref="DrowZzzClock"/> 内に直書きされていた数値リテラルを
    /// 命名された <c>const</c> として切り出す。Domain ではなく Application 層に配置するのは、
    /// 21:00 開始 / 1 ターン 30 分 / 21 ターン構成が DrowZzz 固有のゲームルールであり Domain ゲーム非依存原則と整合させるため。
    /// <para>
    ...
    {
        /// <summary>ゲーム開始時刻 (時、24 時間制)。21:00 開始(L2)。</summary>
        ...
        /// <summary>1 ターンあたりのフェーズ数(L2、N=2 想定)。N&gt;2 拡張は Phase 3 候補。</summary>
        public const int PhasesPerRound = 2;

        /// <summary>夜の開始ターン番号(21:00 = Round 1、L2)。</summary>
        public const int NightStartRound = 1;

        /// <summary>夜の終了ターン番号(04:30 = Round 16、L2)。</summary>
        public const int NightEndRound = 16;

        /// <summary>朝の開始ターン番号(05:00 = Round 17、L2)。</summary>
        public const int MorningStartRound = 17;

        /// <summary>朝の終了ターン番号(07:00 = Round 21、最終プレイ可能ターン、L2)。</summary>
        ...
        /// 意味的には「最終プレイ可能ターン」と同値だが、ゲーム終了判定の文脈で
