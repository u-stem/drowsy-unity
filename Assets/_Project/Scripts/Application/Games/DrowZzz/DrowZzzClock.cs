namespace Drowsy.Application.Games.DrowZzz
{
    ...
    /// を `RoundNumber` から派生する computed プロパティとして提供する。
    /// </summary>
    /// <param name="RoundNumber">
    /// ゲーム内ターン番号(1 起点)。<see cref="DrowZzzGameSession.Clock"/> プロパティで
    /// <c>(TurnState.TurnNumber + 1) / 2</c> を評価した値が渡される。
    ...
    /// <item><c>RoundNumber &gt; 21</c> は時計仕様上存在しないが、<c>IGameRule.IsTerminated</c> が
    ///       明示的にガードするまでの過渡期間は computed プロパティとして数学的計算結果を返す。
    ///       <c>IsNight</c> / <c>IsMorning</c> は両方 <c>false</c> を返すことで「夜でも朝でもない」防御値となる。</item>
    /// <item>「1 ターン = 30 分 = N=2 フェーズ」の前提に基づくため、N&gt;2 拡張時は計算式の再評価が必要(Phase 3 候補)。</item>
    /// </list>
    /// 用語規約:「ターン (Turn)」= 30 分の大単位、本クラスの <c>RoundNumber</c> に対応。
    /// </remarks>
