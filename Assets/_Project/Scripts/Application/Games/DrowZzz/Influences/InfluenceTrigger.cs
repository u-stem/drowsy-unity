namespace Drowsy.Application.Games.DrowZzz.Influences
{
    ...
    /// </summary>
    /// <remarks>
    /// <c>OwnPhaseStart</c> に加えて Reactive Influence(アクション後発動型)対応として
    /// <c>OnOwnPlayCardAfter</c> / <c>OnOwnAbandonAfter</c> を提供する(No.12「偽りの太陽」)。
    /// 将来後続効果カードで他のトリガー(<c>OpponentPhaseStart</c> / <c>OwnPhaseEnd</c> / <c>RoundEnd</c> 等)が
    /// 登場した時点で値を追加する(JIT 拡張方針)。
    /// <para>
    /// 用語規約:
    /// <list type="bullet">
    ...
    public enum InfluenceTrigger
    {
        ...
        /// <summary>
        /// 影響保有者の <c>PlayCardAction</c> 実行直後に発動する(No.12「偽りの太陽」)。
        /// </summary>
        /// <remarks>
        /// 検出箇所: <c>DrowZzzRule.ApplyPlayCard</c> の末尾で、当該アクション **開始時** の influences snapshot に対して walk
        /// (本アクションで新規付与された影響は snapshot 外で対象外)。
        /// </remarks>
        OnOwnPlayCardAfter,

        /// <summary>
        /// 影響保有者の <c>AbandonAction</c> 実行直後に発動する(No.12「偽りの太陽」)。
        /// </summary>
        /// <remarks>
