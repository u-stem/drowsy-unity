...
namespace Drowsy.Application.Persistence
{
    ...
    /// </summary>
    /// <remarks>
    /// ADR-0016 §5.2「<c>IDrowZzzGameSessionSerializer</c> interface 抽出」で確定。
    /// Application 層が interface を定義し Infrastructure 層が具象を実装する Ports &amp; Adapters パターン
    /// (CLAUDE.md §5 アーキテクチャ依存ルール準拠)。
    /// <para>
    /// 同期 API(<see cref="Save"/> / <see cref="Load"/>)は M4-PR5 既存実装の 43 テスト互換のため維持し、
    /// 非同期 API(<see cref="SaveAsync"/> / <see cref="LoadAsync"/>)を M5-PR1 で追加する。引数順は
    /// 既存 Infrastructure 実装と完全一致(session-first / path-only)を維持(ADR-0016 §5.2 W-4 反映)。
    /// </para>
    /// <para>
    /// <see cref="LoadAsync"/> の戻り値は <see cref="UniTask{T}"/>(non-null)。ファイル不在は
    /// <see cref="FileNotFoundException"/> で表現する(ADR-0015 NRT 不採用方針整合、ADR-0016 §5.2)。
    /// </para>
    /// <para>
    ...
    /// Project Singleton 化する(ADR-0016 §5.2 / §7.2 W-1 / W-5)。
    /// </para>
    /// </remarks>
