...
- **Phase 2**(DrowZzz 本命実装): **進行中**(ADR-0005、M1〜M5)
  ...
  - **M4**(永続化 / SO 化 + ユーザー設定): **進行中** — ADR-0012 起票済、M4-PR1(ScriptableObjectCardCatalog 骨格 + Infrastructure.Tests 新設)/ M4-PR2(IEffect の SO 表現 + AdjustSdpEffectAsset)/ M4-PR3(残り 11 派生型 + wrapper 再帰 + 中間型 2 件、SO 表現基盤完成、INF-019 graceful skip 本格化)完成、CLI 機械検知レイヤ整備(PR #63、dotnet build pre-commit + Unity CLI script)、M4-PR4(既存 3 カード No.00 / No.01 / No.02 の SO Asset 化)着手予定
