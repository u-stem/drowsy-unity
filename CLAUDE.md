...
- **Phase 2**(DrowZzz 本命実装): **進行中**(ADR-0005、M1〜M5)
  ...
  - **M5**(Bootstrap / Presentation): **進行中**(ADR-0016、M5-PR1〜PR7 完成 + M5-PR8 着手済。VContainer 2 階層 LifetimeScope + MVP(Pure C# Presenter)+ UI Toolkit + R3 `Subject<T>` + UniTask 永続化)。M5-PR7 マージ後の Unity Play モード実機検証で根本問題 2 件が連鎖発覚し、**PR #94**(scene-setup.md 手順書)/ **PR #95 ADR-0017**(PlayerRoster wrapper、VContainer `IReadOnlyList<T>` 予約型回避)/ **PR #96 ADR-0018**(CardTypeId 新設 + CardId 複合型 refactor、Hand 重複検出エラー根本対処、Drowsy.Domain 100% カバレッジ)で対処済。M5-PR8(Phase 2 完結 PR、`docs/m5-pr8-phase2-completion` ブランチ、本 ADR-0016 §11 起票済)で WebGL Build 検証 + Phase 2 完結処理(検証結果反映後に「**完結**」へ更新)
