...
- **M2-PR5**(2026-05-11、進行中): §1.5「継続影響(Influence)」を新規昇格 JIT 確定。`PlayerInfluence` 値オブジェクト + `Apply`/`RemoveInfluenceEffect` + `ChoiceEffect` + `EffectContext` 3 引数 overload + 自フェーズ開始時 Tick(`DrowZzzRule.ApplyEndTurn` 内)、`PlayCardAction(CardId, int Choice = 0, int InfluenceRemovalIndex = 0)` 拡張
...
- **M2-PR4**(PR #37):
  ...
  - NUnit +33 件 / TestCase 展開で +41 ケース
- **M2-PR5**(進行中):
  - 継続影響(Influence)機構: `PlayerInfluence` 値オブジェクト + `InfluenceTrigger` enum(`OwnPhaseStart` のみ)+ `DrowZzzGameSession.Influences` 拡張(cross-field 検証 + Equals 順序保持シーケンス同値)
  - 影響系効果 record: `ApplyInfluenceEffect` / `RemoveInfluenceEffect` + `EffectInterpreter.Apply` 3 引数 overload(`EffectContext` 経由で `InfluenceRemovalIndex` を thread)
  - 選択式カード: `ChoiceEffect` record(`DrowZzzRule.ApplyPlayCard` 内で `PlayCardAction.Choice` を読んで unwrap、interpreter には届かない)
  - 自フェーズ開始時 Tick: `DrowZzzRule.ApplyEndTurn` 内 DDP 抽選後に新 current player の影響を FIFO で評価、`TickEffect` 適用 + `RemainingCount-1` + 0 で除去
  - `PlayCardAction` を 3 引数化(`CardId Card, int Choice = 0, int InfluenceRemovalIndex = 0`、default で後方互換)
  - カード No.02「緑の侵攻」: 選択 1(自分 SDP -6 + 相手の影響削除 + 相手に影響付与)/ 選択 2(相手 SDP +6 + 自分の影響削除 + 自分に影響付与)、影響の Tick 効果 = SDP -5、カウント 3
  - ADR-0007 §1.5「継続影響(Influence)」JIT 確定同梱(§8「M2 範囲外」から昇格)
  - 用語規約整合: カードフレーバー「自分のターン開始時」を実装上「フェーズ開始時」(ADR-0009 用語規約)に揃え、enum 値は `OwnPhaseStart`
- **M2-PR6 以降**: 後続効果カードをサブセット先行、`ScriptableObjectCardCatalog` 化は M4 に送る、山札枯渇は現状想定下では発生しない計算根拠を ADR-0007 §「山札枯渇」に明記
