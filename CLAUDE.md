...
- **Phase 2**(DrowZzz 本命実装): **進行中**(ADR-0005、M1〜M5)
  ...
  - **M4**(永続化 / SO 化 + ユーザー設定): **進行中** — ADR-0012 起票済、M4-PR1(SO catalog 骨格)/ M4-PR2(EffectAsset + AdjustSdp)/ M4-PR3(残り 11 派生型 + wrapper 再帰 + 中間型 2 件、SO 表現基盤完成)/ M4-PR4(既存 3 カード No.01 / No.02 / No.00 の SO ↔ InMemory 同値性検証、43 テスト)完成、CLI 機械検知レイヤ整備(PR #63)、M4-PR5(`DrowZzzGameSession` JSON 永続化、初期推奨 Newtonsoft.Json)着手予定
