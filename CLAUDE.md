...
| [ADR-0022](docs/adr/0022-reactive-influence-trigger-extension.md) | InfluenceTrigger 拡張 — Reactive Influence(アクション後発動型)の追加 | No.12「偽りの太陽」の「使用したら SDP-10 / 放棄したら SDP+5」セマンティクス導入のため `InfluenceTrigger` enum を拡張。`OnOwnPlayCardAfter` / `OnOwnAbandonAfter` 2 値を追加し、`ApplyPlayCard` / `ApplyAbandon` の末尾で snapshot ベース walk(本アクションで新規付与された影響は対象外)。新規 effect 2 件(`AdjustSdpAfterPlayCardEffect(int Delta)` / `AdjustSdpAfterAbandonEffect(int Delta)`)。既存 `OwnPhaseStart` カード(No.02/04/06/07/08/09/10/11)への影響なし、`DecrementInfluencesForCurrentPlayer`(ADR-0020)は全 Influences 一律 -1 で Reactive も同様(Perpetual で実質除去なし)|
