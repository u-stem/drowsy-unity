...
**Application/DrowZzz の分類指針(post-Phase2 レビュー W-4 反映)**: `DrowZzzRule.Apply` や `EffectInterpreter` を経由する複合フローを検証するテストは原則 `Small` で運用する(全て in-memory + 数 ms 〜 数十 ms オーダーで完了する Pure C# のため)。`Medium` に分類するのは ScriptableObject の Asset ロード経路を実際にロードする `Infrastructure.Tests` や、Persistence で実ファイル I/O を行う `DrowZzzGameSessionSerializerTests` 系のみ。Integration 名のテストでも in-memory なら `Small` 扱い。
