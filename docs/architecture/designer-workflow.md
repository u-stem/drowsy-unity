...
### 2-3. polymorphic 編集(`[SerializeReference]`)

`Effects` フィールドは `EffectAsset` 基底の派生型を `[SerializeReference]` で持つため、Inspector で **要素ごとに型を選択** する。

- 配列の `+` ボタンを押すと「Add element」で型選択メニューが出る
- 型を選ぶと該当 派生型の Inspector が展開される
- wrapper 効果(`TimeOfDayBranchEffectAsset` / `ChoiceEffectAsset` / `KeywordedEffectAsset`)の内部 `EffectAsset[]` も同様に polymorphic 編集される
