...
- [INF-156] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"13"`「最後の砦Ⅰ」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-17、`AssociateSpecificCardEffect(CardTypeId)` 初導入(カード効果による自動連想、Hand Instance 採番 + AssociatedCardIds 記録)
  - SO 経路:`ChoiceEffectAsset(branches: [Choice0=AdjustSdp ×2, Choice1=AdjustSdp ×2 + AssociateSpecificCardEffectAsset("14")])` の 1 件最上位
- [INF-157] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"14"`「最後の砦Ⅱ」, name / effects shall equal `InMemoryCardCatalog`.
  - SO 経路:`ChoiceEffectAsset(branches: [Choice0=AdjustSdp ×2, Choice1=AdjustSdp ×2 + AssociateSpecificCardEffectAsset("15")])` の 1 件最上位
- [INF-158] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"15"`「最後の砦Ⅲ」, name / effects shall equal `InMemoryCardCatalog`.
  - SO 経路:`ChoiceEffectAsset(branches: [Choice0=AdjustSdp ×2, Choice1=AdjustSdp ×2 ※連想 effect なし(終端)])` の 1 件最上位
- [INF-160] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"16"`「自分勝手な審判」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-17、`ConditionalApplyOrClearInfluencesEffect(SdpTarget, int Threshold, PlayerInfluence)` 初導入(条件分岐 effect、Count <= Threshold で Apply、> Threshold で Clear)
  - SO 経路:`ChoiceEffectAsset(branches: [Choice0=AdjustSdp(Self,-8) + AdjustSdp(Opp,+5) + ConditionalApplyOrClearInfluencesEffectAsset(Self, 2, PlayerInfluenceAsset(OwnPhaseStart, AdjustSdpEffectAsset(Self,-4), Perpetual)), Choice1=AdjustSdp(Self,+5) + AdjustSdp(Opp,-8) + Conditional(Opp, 2, 同 PlayerInfluenceAsset)])` の 1 件最上位
  - record 値同値:ChoiceEffect 2 分岐 + 各分岐 3 件順序保持 + `ConditionalApplyOrClearInfluencesEffect.Target/Threshold/InfluenceToApply` 同値(InfluenceToApply は PlayerInfluence auto-equals + 内部 TickEffect record 同値)
- [INF-162] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"17"`「見掛け倒しの障壁」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-18、既存 Counter キーワード機構(M3-PR5b/c、ADR-0011 §4.3)初の本物カード(過去はテスト用ダミー `c_counter` のみ)、新規 effect 不要
  - SO 経路:`KeywordedEffectAsset([Counter], AdjustSdpEffectAsset(Self, 0))` 1 件最上位、Inner はダミー(`KeywordedEffect.Inner` null 禁止のため SDP 変動 0 で代用)
  - record 値同値:`KeywordedEffect.Keywords[Counter] + Inner = AdjustSdpEffect(Self, 0)` の record 同値(Counter キーワード性質 `HasKeywordInEffects(effects, Counter)` が両 catalog で true 返却)
- [INF-163] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"18"`「対抗手段」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-18、ADR-0023(Echo キーワード初導入)、`Keyword.Echo` + `ReuseInfluenceSourceEffect` + `PlayerInfluence.OriginEffects` 追加と同 PR、`ReuseInfluenceSourceEffectAsset` 新規 SO 追加
  - SO 経路:`KeywordedEffectAsset([Echo], ReuseInfluenceSourceEffectAsset())` 1 件最上位、Inner はマーカー(`KeywordedEffect.Inner` null 禁止 + `ReuseInfluenceSourceEffect` は単体で意味を成すため Inner = 本マーカーとして自然に成立)
  - record 値同値:`KeywordedEffect.Keywords[Echo] + Inner = ReuseInfluenceSourceEffect()` の record 同値(Echo キーワード性質 `HasKeywordInEffects(effects, Echo)` が両 catalog で true 返却)
- [INF-164] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"19"`「絶対障壁」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-18、ADR-0024(ゲーム開始時自動連想 marker 初導入)、`AssociateToFirstPlayerOnGameStartEffect` 追加 + `StartGameUseCase` ctor 3 引数化と同 PR、`AssociateToFirstPlayerOnGameStartEffectAsset` 新規 SO 追加
  ...
  - record 値同値:`KeywordedEffect.Keywords[Counter, Frenzy] + Inner = AdjustSdpEffect(Self, 0)` + `AssociateToFirstPlayerOnGameStartEffect()` の 2 件、Counter + Frenzy 両キーワード性質 `HasKeywordInEffects(effects, Counter)` / `HasKeywordInEffects(effects, Frenzy)` が両 catalog で true 返却
- [INF-165] When `ScriptableObjectCardCatalog` is constructed with the SO representation of Card `"20"`「至上の喜び」, name / effects shall equal `InMemoryCardCatalog`.
  - 関連:2026-05-18、ADR-0025(プレイ時 / 放棄時分岐 wrapper 初導入)、`PlayOrAbandonBranchEffect` 追加 + `DrowZzzRule.ApplyAbandon` catalog 依存追加と同 PR、`PlayOrAbandonBranchEffectAsset` 新規 SO 追加(`TimeOfDayBranchEffectAsset` 同パターン)
  - SO 経路:`PlayOrAbandonBranchEffectAsset` 1 件最上位、PlayEffects 3 件(`AdjustSdpEffectAsset(Self, +20)` + `AdjustSdpEffectAsset(Opp, -20)` + `ApplyInfluenceEffectAsset(Self, PlayerInfluenceAsset(OwnPhaseStart, RestrictAllUsageAndAbandonInfluenceMarkerEffectAsset, 1))`)+ AbandonEffects 2 件(`AdjustSdpEffectAsset(Self, +4)` + `AdjustSdpEffectAsset(Opp, +6)`)
  - record 値同値:`PlayOrAbandonBranchEffect` の順序保持シーケンス Equals(`TimeOfDayBranchEffect` と同パターン)で PlayEffects / AbandonEffects 全要素一致
...
| INF-156 | `LastBastion1CardCatalogTests.Given_*` 2 件 | ChoiceEffect 2 分岐、Choice1 に AssociateSpecificCardEffectAsset("14") 含む、自動連想 effect の SO 経路初導入 |
| INF-157 | `LastBastion2CardCatalogTests.Given_*` 2 件 | ChoiceEffect 2 分岐、Choice1 に AssociateSpecificCardEffectAsset("15") 含む |
| INF-158 | `LastBastion3CardCatalogTests.Given_*` 2 件 | ChoiceEffect 2 分岐、Choice1 に連想 effect なし(終端カード) |
| INF-160 | `SelfishJudgementCardCatalogTests.Given_*` 2 件 | ChoiceEffect 2 分岐、各分岐 3 件(AdjustSdp ×2 + ConditionalApplyOrClearInfluences)、条件分岐 effect の SO 経路初導入 |
| INF-161 | (本表対象外) | No.16 の JSON round-trip、persistence 要件として `drowzzz-game-session-serializer.md` に分離(catalog 同値性ではないため card-catalog.md 表外、code-reviewer W-2 反映 2026-05-18)|
| INF-162 | `FacadeBarrierCardCatalogTests.Given_*` 2 件 | KeywordedEffect([Counter], AdjustSdpEffect(Self, 0)) 1 件最上位、Counter キーワード機構の本物カード SO 経路初導入(過去はテスト用ダミー `c_counter` のみ) |
| INF-163 | `CountermeasureCardCatalogTests.Given_*` 2 件 | KeywordedEffect([Echo], ReuseInfluenceSourceEffect()) 1 件最上位、Echo キーワード初導入(ADR-0023)、`ReuseInfluenceSourceEffectAsset` 新規 SO の SO 経路初導入 |
| INF-164 | `AbsoluteBarrierCardCatalogTests.Given_*` 2 件 | KeywordedEffect([Counter, Frenzy], AdjustSdpEffect(Self, 0)) + AssociateToFirstPlayerOnGameStartEffect() の 2 件最上位、ADR-0024 ゲーム開始時自動連想 marker 初導入、`AssociateToFirstPlayerOnGameStartEffectAsset` 新規 SO + Counter+Frenzy 両キーワード持ち SO 経路初導入 |
| INF-165 | `SupremeJoyCardCatalogTests.Given_*` 2 件 | PlayOrAbandonBranchEffect 1 件最上位、ADR-0025 プレイ時 / 放棄時分岐 wrapper 初導入、`PlayOrAbandonBranchEffectAsset` 新規 SO(TimeOfDayBranchEffectAsset 同パターン)、PlayEffects 3 件 (AdjustSdp ×2 + ApplyInfluence(Self, RestrictAllUsageAndAbandon Marker count=1)) + AbandonEffects 2 件 (AdjustSdp ×2) |
