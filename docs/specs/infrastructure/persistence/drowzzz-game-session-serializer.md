...
- [INF-159] The `EffectJsonConverter` shall round-trip `AssociateSpecificCardEffect(CardTypeId TargetCardTypeId)`(No.13/14/15「最後の砦Ⅰ/Ⅱ/Ⅲ」、2026-05-17 で導入)via the `"type"` discriminator(`"AssociateSpecificCard"`)+ `targetCardTypeId` フィールド(string、CardTypeId.Value 相当、既存 `RestrictSpecificCardInfluence` 同様のキー名に統一、code-reviewer W-1 反映 2026-05-17)。`CardTypeId.Of(string)` 経由で復元(default ctor 不在のため、`RestrictSpecificCardInfluenceEffect` と同パターン)。
- [INF-161] The `EffectJsonConverter` shall round-trip `ConditionalApplyOrClearInfluencesEffect(SdpTarget Target, int Threshold, PlayerInfluence InfluenceToApply)`(No.16「自分勝手な審判」、2026-05-17 で導入)via the `"type"` discriminator(`"ConditionalApplyOrClearInfluences"`)+ `target`(SdpTarget enum)+ `threshold`(int)+ `influenceToApply`(PlayerInfluence、内部 TickEffect は本 converter で再帰 dispatch)。`ApplyInfluenceEffect` の `influence` フィールドと同パターンで PlayerInfluence の再帰 serialize/deserialize。
...
| INF-159 | `EffectJsonConverterTests.Given_AssociateSpecificCardEffect_When_RoundTrip_Then_等価` | No.13/14/15「最後の砦Ⅰ/Ⅱ/Ⅲ」追加派生型 round-trip(TargetCardTypeId フィールド、string serialize → `CardTypeId.Of` 復元)|
| INF-161 | `EffectJsonConverterTests.Given_ConditionalApplyOrClearInfluencesEffect_When_RoundTrip_Then_等価` | No.16「自分勝手な審判」追加派生型 round-trip(Target / Threshold / InfluenceToApply、PlayerInfluence 再帰 dispatch)|
