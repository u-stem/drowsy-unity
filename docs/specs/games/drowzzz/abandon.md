...
## 「本能(Instinct)」キーワードとの関係(M3-PR5a で機能化)

M3-PR3 段階では `CardIndex` 制約は「範囲内 + 手札非空 + (RepairBed なら BedDamages > 0%)」のみ。**M3-PR5a で「本能(Instinct)」キーワードを持つ効果列を含むカードを `IsLegalMove` で除外する追加制約** を追加(DZ-213、ADR-0011 §4.2)。判定は `DrowZzzRule.HasInstinctKeyword` ヘルパーで再帰的に効果列を walk(`TimeOfDayBranchEffect` / `ChoiceEffect` の nest にも対応)。

- [DZ-213] When `AbandonAction(_, CardIndex)` is evaluated and the effect list of `Hand.Cards[CardIndex]` contains a `KeywordedEffect` with `Keyword.Instinct`(検出は `DrowZzzRule.HasInstinctKeyword` で再帰判定、`TimeOfDayBranchEffect.NightEffects` / `MorningEffects` / `ChoiceEffect.Branches` の nest 内部も対象)、`IsLegalMove` shall return `false`. `Apply` 経路で違反すると `InvalidOperationException`。
...
| DZ-213 | Instinct カードを CardIndex 対象 5 件(c1 Instinct で false / c2 normal で true / Apply で例外 / TimeOfDayBranch nest で false / ChoiceEffect nest で false) | Instinct 除外、再帰判定(top-level / wrapper nest 2 種)|
