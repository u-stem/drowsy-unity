...
| `EndTurnAction` | 可 | `WaitingForEndTurn` フェーズで常に合法(進行不能化回避)|
...
- [DZ-311] When B holds the marker influence and B tries to perform `EndTurnAction` during `WaitingForEndTurn`, `IsLegalMove` shall return `true`(進行不能化回避、本 marker は EndTurnAction を illegal 化しない)。
...
| DZ-311 | `Given_p2が本Markerカウント1保有_When_p2がEndTurnActionでIsLegalMove_Then_true` | EndTurnAction は許可(進行不能化回避)|
