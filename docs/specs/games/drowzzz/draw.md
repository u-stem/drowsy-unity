...
ADR-0006 §2.4 / §M1-PR4 の決定に基づく。`DrawCardAction` は `WaitingForDraw` フェーズで現プレイヤーが行う「山札から手札に 1 枚移動」のアクション。Apply 後 `TurnPhase` は `WaitingForPlay` に遷移する。プレイヤー Id は payload に持たず、`session.GameState.Turn.CurrentPlayerIndex` から暗黙取得する (ADR-0006 §2.1)。
...
- [DZ-038] When `IsLegalMove(session, DrawCardAction)` is called and `session.TurnPhase == WaitingForDraw`, then it shall return `true`.
- [DZ-039] When `IsLegalMove(session, DrawCardAction)` is called and `session.TurnPhase != WaitingForDraw`, then it shall return `false`.
...
- [DZ-043] When `Apply(session, DrawCardAction)` is called and `IsLegalMove` returns `true`, then `result.TurnPhase` shall be `WaitingForPlay`.
...
- [DZ-046] If `Apply(session, DrawCardAction)` is called and `IsLegalMove` returns `false` (i.e. `session.TurnPhase != WaitingForDraw`), then it shall throw `InvalidOperationException`.
...
| DZ-043 | `Given_合法状態_When_DrawCardActionをApply_Then_TurnPhaseがWaitingForPlayに遷移する` | |
