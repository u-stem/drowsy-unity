...
| ---- | ---- | ---- | ---- | ---- |
| `Frenzy`(狂乱) | 反撃を受けない | enum 値のみ宣言 | `CounterAction.IsLegalMove` で対象判定に利用 | — |
| `Instinct`(本能) | 手段の放棄を受け付けない | **機能化済**(`AbandonAction.IsLegalMove` で除外) | — | — |
| `Counter`(反撃) | 相手のカードを無効化 | enum 値のみ宣言 | `CounterAction` / `WaitingForCounterResponse` 追加 | 反撃の反撃 + 元カード遡及発動 |
