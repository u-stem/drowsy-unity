...
### 補足:`SdpTarget` enum を「プレイヤー解決」に流用する Tech Debt(2026-05-17 No.05 追加時 code-reviewer W-1 反映)

`SdpTarget` enum は本来 SDP(SecondDrowsyPoints)操作専用(`SdpTarget.cs` xmldoc に「将来 `EffectTarget` 改名 or 別 enum 新設を再評価」と既記録)だが、PR ②(`ApplyTargetedRestrictionEffect`)+ No.05(`StackHandCardOnDeckTopEffect`)+ `IsLegalTargetCardId` 共通ヘルパーで「プレイヤー解決」semantics への流用が **3 経路に拡大**。

放置すると将来カード設計者が「`SdpTarget.Self` を渡せばいい」と semantic 違反を自然に写経する温床になる。

対処方針(本 PR では実装せず tracking のみ):
- `SdpTarget` → `EffectTarget` 改名 + 全 effect record + EffectAsset + Bootstrap + EARS + tests の機械的 rename
- 影響範囲が広いため、別 chore PR(候補:`chore/sdp-target-rename-to-effect-target`)で一括処理する方が筋
- `docs/todo.md` 肥大化回避方針との整合上、本 ADR §補足で tracking(SoT 集約)
