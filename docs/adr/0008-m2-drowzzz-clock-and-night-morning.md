...
**M2-PR3 JIT 確定(2026-05-11、ADR-0009「コップ一杯の脅威」JIT 共有時)**: 「**条件発動 record にラップする**」案を採用、すなわち `TimeOfDayBranchEffect(IReadOnlyList<IEffect> NightEffects, IReadOnlyList<IEffect> MorningEffects)` ラッパー record を 1 つ導入し、カード Effects 列に 1 個入れることで「夜・朝で完全に違う効果列」を表現する。コップ一杯の脅威(夜は SDP↓ + ドロー / 朝は SDP↑)のように **同一カードが時刻反転で別効果を持つ** ケースを 1 ストラクチャで読めることを優先(夜限定 / 朝限定カードも `NightEffects: []` / `MorningEffects: []` で表現可能)。「効果自身が判定」案は同種別 record が爆発するため不採用、「ICardCatalog 条件付き返却」案は `ICardCatalog` シグネチャ再設計が必要で ADR-0007 §3 / §6 と非互換のため不採用。両者 `false` の時刻(Round 22+ 過渡的範囲、本 ADR §5)では no-op(session 不変返却、DZ-122)。詳細は `docs/specs/games/drowzzz/effects/time-of-day-branch.md` 参照。
