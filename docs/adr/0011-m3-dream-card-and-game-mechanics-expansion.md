...
> **Note(M3 完結の帰結、2026-05-16)**:本 ADR の 6 機構(連想 / 放棄 / ベッド破損 / キーワード能力 / ターン構造詳細化 / カード No.00「夢」)は M3-PR2〜PR6 で実装が完結し、Phase 2 完結時点で M3 ステータスを **完結扱い** とする(`CLAUDE.md` §11 / [ADR-0005 §7](0005-phase2-roadmap-drowzzz.md) / [ADR-0016 §11 M5-PR8 完成記録](0016-m5-bootstrap-presentation.md))。
...
順序は **ADR-0010 §4「順序保証」を維持** しつつ、本 ADR §3 のベッド破損ダメージを最先頭に挿入。ADR-0020(2026-05-17)で `Turn.Next` 前に **「現プレイヤー Influences の RemainingCount 減算」** ステップを追加:

0. **Influence Decrement**(ADR-0020 追加、`ApplyEndTurn` 冒頭、`Turn.Next` 前、旧 current プレイヤーの全 Influences を count -1、0 で除去)
1. **ベッド破損 SDP マイナス**(本 ADR §3、Turn.Next 後の新 current プレイヤーに対して)
2. 新フェーズ確定(`Turn.Next`)— 既存
3. PhaseState 更新 — 既存
4. DDP 抽選(該当ラウンド)— M2-PR4
5. 影響 Tick — M2-PR5(ADR-0020 後は **TickEffect 適用のみ、count 不変**)
6. Round 21 完了検出 → `Outcome` 設定 — M3-PR1

ベッド破損ダメージを最先頭に置く根拠:JIT 共有「自分のターン開始時、まずベッドの破損による SDP のマイナスが入ります」の「まず」と整合。

ADR-0020 で `Influence Decrement` を `Turn.Next` 前に置く根拠:「カウント N = 自フェーズ N 回機能」を Marker 系(`IsLegalMove` で「存在時」効果を発揮)で成立させるため、自フェーズ全体で Influence が保有されている状態を維持し、フェーズ終了時に -1 する。詳細は ADR-0020 §1 を参照。
...
- ScriptableObject 化(M4)は本 ADR の範囲外、`Keyword` enum / ベッド破損計算式の SO 移行は M4 で別 ADR で再評価 → **ADR-0012(M4 詳細)§10「不採用案」で不採用確定**:L2 数学定数(CLAUDE.md §9)は SO 化対象外、SO 化は L3 ゲームバランス値のみが対象(`IGameConfig` の `FdpPool` / `DdpPool` 等)
