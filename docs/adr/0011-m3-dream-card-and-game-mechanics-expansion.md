...
| ベッド破損計算 | ダメージ式は **JIT 確定済**(5% につき -1、`bedDamage / 5`)。残課題: 破損率増加トリガー(どの行動で、何 % 増えるか)|
...
破損率が 5 の倍数でない場合(例:`7%`)は整数除算で **切り捨て**(`7 / 5 = 1` → SDP -1)。破損率の増減幅(`+5% / +10% / +20%` 等)が 5 の倍数で設計されているか、5 の倍数以外も発生するかは、§3 「破損率増加トリガー」(JIT 確認待ち)の確定時に再確認する。
