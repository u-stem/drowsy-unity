...
| **狂乱** | 「反撃」を受けない効果 |
| **本能** | 手段の放棄を受け付けない効果 |
| **反撃** | ターン中に使用された手段に対してこの手段を使用することで相手が使用した手段を無効化する。同じ「反撃」を持つカードに対しても適用 |
| **放棄** | プレイヤーはターン中、手段を使うか、放棄するか選択。放棄時:手段を捨てて SDP +5、または ベッドを 20% 修繕、のどちらかを選択 |
| **ベッド破損率** | 0〜100%。破損ダメージとして毎ターン SDP マイナス(20% で -4、40% で -8、…)|
...
| 機構 | 未確定の詳細 |
| ---- | ---- |
| 連想 | 連想対象の領域(山札 top / 特定位置 / 別領域)、FDS 80「超」(81+)か「以上」(80+)か、連想タイミング(自ターン / 相手ターン)|
| 反撃 | 「反撃」を「反撃」で打ち消した時の元カードの扱い、効果スタック順、相手ターン中のカードプレイ機構の有無 |
| 本能 | 「放棄を受け付けない」の正確な意味(放棄選択画面でこのカードを捨てられない? 別の意味?)|
| ベッド破損計算 | ダメージ式(20% で -4、40% で -8 → 線形 / 別)、破損率増加トリガー(どの行動で、何 % 増えるか)|
...
- `action.Card` が連想可能なカード(JIT 確認待ち:M3 範囲では「夢」のみ?)
...
| (c) 山札の外の特別領域(catalog / 別 Pile)| カードが山札に含まれない設計、最も独立した「夢」専用 |
| (d) `ICardCatalog` から直接生成 | 「夢」が初期山札に含まれず、連想で初登場 |

**初期推奨**: (c) または (d)。「夢」が通常山札に紛れない方が「特殊な手段」という JIT 共有原文と整合。M3-PR(連想実装)着手時に JIT 確定。
...
#### 自ターン開始時の SDP マイナス計算(JIT 確認待ち)

| 候補式 | 例 | 評価 |
| ---- | ---- | ---- |
| (a) **線形** `damage = (bedPct / 20) * 4`(20% で -4、40% で -8、60% で -12、80% で -16、100% で -20)| `(40 / 20) * 4 = 8` | JIT 共有「20% で -4、40% で -8 のように」と直接整合(暗示で線形)|
| (b) 累積式(20% で -4、40% で -12、60% で -24)| `4 + 8 = 12` | 「のように」が累積を示唆する解釈 |
| (c) 別の非線形式 | — | JIT 確認待ち |

**初期推奨**: (a) 線形。JIT 共有原文「20% で SDP -4、40% で SDP -8 というように」が「次の段階は -12、-16、-20 と続く」の暗示として最も自然。

#### 破損率増加トリガー(JIT 確認待ち)

JIT 共有「行動による何かしらが発生して、このタイミングでベッドにもダメージが入るなら破損率を上げます」とあり、**どの行動で / 何 % 増えるか** は未確定。候補:

- 候補 A: 特定カードプレイ時に破損
- 候補 B: 放棄(SDP+5 を選んだ時)に破損
- 候補 C: 一律にターン経過で破損(自動進行)
- 候補 D: 効果 record に「ベッド破損効果」(`DamageBedEffect`)を新設、カード固有

**初期推奨**: 候補 D(`DamageBedEffect(SdpTarget, int Percent)`)。M2-PR3 / M2-PR5 で確立した「カード効果は `IEffect` で表現」と整合、specific card で破損トリガーを持つ。
...
| 案 | 評価 |
| ---- | ---- |
| (a) 効果 record の属性として持つ(`AdjustSdpEffect(SdpTarget, int Delta, KeywordSet Keywords)`)| 全 effect record にフィールド追加、影響大 |
| (b) **新規ラッパー record**(`KeywordedEffect(IReadOnlyList<Keyword> Keywords, IEffect Inner)`)で wrap | **採用候補**。`TimeOfDayBranchEffect` / `ChoiceEffect` と同パターン(既存 effect を wrap)|
| (c) カードレベルで持つ(`CardData.Keywords`)| 効果単位の付与とずれる(夜効果のみ「狂乱・本能」を持つ夢カードのケースで困難)|
| (d) 別軸の属性(`KeywordAttribute` enum を `CardData.Attributes` に格納)| `CardData.Attributes` の汎用 dict 流用で型安全性低、ADR-0007 §1 と矛盾 |

**初期推奨**: (b) `KeywordedEffect` wrapper。`TimeOfDayBranchEffect` / `ChoiceEffect` で確立した「効果列 wrapper」パターンを継承(ADR-0007 §1 / ADR-0010 §5 の「カード効果は IEffect で表現」と整合)。
...
「**ターン中に使用された手段に対してこの手段を使用することで相手が使用した手段を無効化**」。新規 `IGameAction` 派生型 **`CounterAction(CardId Counter, CardId Target)`** を追加(JIT 確認待ち:相手のターン中に自分のカードをプレイできる機構の追加 = 既存「自分のターン中のみカードプレイ可能」原則を覆す)。

##### 4.4. 反撃 vs 反撃 / 反撃 vs 狂乱

- 反撃を反撃で打ち消し: JIT 共有「同じ「反撃」を持つカードに対しても適用」より、反撃の反撃が成立する。元カードはどうなるか不明(JIT 確認待ち)
- 反撃 vs 狂乱: 狂乱は反撃を受けない、つまり狂乱を持つ効果に対しては反撃 illegal
...
// effects 側(KeywordedEffect で「狂乱・本能」をラップ)
...
| PR | 内容 | 主な追加・変更 | 依存 |
| ---- | ---- | ---- | ---- |
| **M3-PR2** | ベッド破損率(Session 状態 + 自ターン開始時のダメージ計算) | `DrowZzzGameSession.BedDamages` 追加(9 引数 ctor 化)、`ApplyEndTurn` 内ダメージ計算ステップ追加、`DrowZzzVictoryConstants` or 新規 constants でダメージ式集約、`DamageBedEffect` record 追加候補 | M3-PR1 |
...
| **M3-PR4** | 連想機構(`AssociateAction` + 特殊ドロー) | `AssociateAction(CardId)` + `IsLegalMove`(FDS ≥ 80 超)+ `Apply`(特殊な手段で手札に追加)、連想テスト | M3-PR1 |
| **M3-PR5** | キーワード能力(狂乱 / 本能 / 反撃) | `Keyword` enum、`KeywordedEffect` ラッパー record、`CounterAction(CardId, CardId)` 追加、`AbandonAction.IsLegalMove` で Instinct チェック | M3-PR3(放棄機構に Instinct 影響)|
...
- ベッド破損ダメージ計算式の確定(線形 / 累積 / 別)
- ベッド破損率増加トリガーの確定(どの行動で / 何 % ずつ)
...
- 連想可能なカードの範囲(M3 では「夢」のみ?)

### M3-PR5 着手時の JIT 確認項目(本 ADR §4 関連)

- 反撃 vs 反撃の元カード扱い(双方無効化 / どちらか一方残る)
- 効果無効化のセマンティクス(候補 A/B/C のどれか)
- 相手ターン中のカードプレイ機構の有無(`CounterAction` を相手ターン中に発動できるか)
