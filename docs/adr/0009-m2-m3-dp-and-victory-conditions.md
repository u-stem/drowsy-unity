...
→ **計 5 回**。各タイミングで N=2 プレイヤー × 1 枚 = 2 枚を共有プールから抽選。総抽選 10 枚 ≤ プール 36 枚で余裕 26 枚。
...
public interface IGameConfig
{
    ...
    IReadOnlyList<int> DdpPool { get; }  // 新規: -30..+30 で 5 刻み × 3 枚 = 36 枚相当の値列
}
```

`IGameConfig.DdpPool` は「13 種 × 3 枚」の構造で 36 要素を持つ静的設定(`StubGameConfig` のデフォルト値で初期化)。`StartGameUseCase` で session に渡す際に `Pile.Shuffle(IRandomSource)` でランダム順に整列し、`DrowZzzGameSession.DdpPool` として保持する。
...
- DDP プール枯渇は現状想定下で発生しない(総抽選 10 ≤ プール 36)が、将来効果で「DDP を追加抽選する」効果が登場した場合は再計算が必要(`docs/todo.md` で追跡)
...
| `Tests/Application.Tests/Stubs/StubGameConfig.cs` | `DdpPool` デフォルト値(13 種 × 3 枚 = 36 要素)|
...
1. **DDP プール枯渇可能性チェック**(`priority: medium`):本 ADR 確定時点では 5 回 × N=2 = 10 枚抽選 ≤ プール 36 枚で余裕 26 枚あるが、将来「DDP を追加抽選する効果」が登場した場合に再計算が必要。M2 各 PR で効果が DDP 抽選回数を増やす変動を含む場合、Self-Review で「総抽選 ≤ プール 36」確認
...
本 ADR 起票 PR では Implementation Notes §M2-PR3〜の完成記録は **空欄のまま**。各 M2-PR 完成時 / M3-PR 完成時に当該 PR で本 ADR に完成記録を追記する(ADR-0007 / ADR-0008 §M2 完成記録の追記タイミング と同パターン)。
