...
| 最大ラウンド数(`MaxRoundNumber`) | 20(M3 で `IGameConfig.MaxRoundNumber` として実装予定、ADR-0006 §1.4) |
| 総 Draw 数(ゲーム終了まで) | 2 枚 × 20 ラウンド = 40 枚 |
| 初期配布 + 総 Draw 数 | 10 + 40 = 50 枚 ≤ 56 枚(余裕 6 枚) |
...
Context §「山札枯渇 / 手札 0 枚」で示した通り、**現状の数値前提下では発生しない**(N=2 × 20 ラウンド × 1 Draw + 初期配布 10 = 50 ≤ 山札 56)。本 ADR では以下を確定する:
...
1. **M2-PR1**: 効果インフラ整備(本 ADR の核心)
   ...
   - EARS: APP-301 等、DZ-301 等(prefix 連番は M2-PR1 で確定)
