...
### M1 着手 PR 群(ADR-0005 の予定を本 ADR で具体化)

1. **M1-PR1**: 汎用 Application 層 interface + Tests asmdef
   - **既存 `Drowsy.Application.asmdef` を流用**(Phase 0 で作成済み)、interface ファイルのみ追加
   - `IGameAction` / `IGameRule<TAction, TSession>` / `ICardCatalog` interface 追加
   - `Drowsy.Application.Tests` asmdef を **新設**(References: `Drowsy.Application`, `Drowsy.Domain`, NUnit)
   - 汎用 interface の単体テスト(必要なら、interface 自体には実装がないため軽め)
2. **M1-PR2**: DrowZzz 固有型の skeleton
   - `DrowZzzAction` 階層(4 種)、`DrowZzzGameSession`、`DrowZzzTurnPhase`、`DrowZzzRule`(NotImplementedException で骨格のみ)
   - `InMemoryCardCatalog`(汎用 stub、Application/Catalog/ に配置)
   - **`IGameConfig` の `FdpPool` 追加は M1-PR3 に回す**(skeleton 段階では NotImpl のため不要、Apply 実装と同時に追加する方が論理変更がまとまる)
3. **M1-PR3**: `StartGameAction` の Apply 実装 + `StartGameUseCase` + `IGameConfig.FdpPool` 追加
   - **`IGameConfig` に `FdpPool` プロパティ追加**(`Drowsy.Domain.Configuration`、interface 拡張 + stub 実装)
   - 先後ランダム決定 / FDP 抽選 / 初期 5 枚配布 / セッション構築
   - EARS / Gherkin: `docs/specs/games/drowzzz/setup.{md,feature}`
   - テスト: N=2 で初期セッションが期待通り構築されること
4. **M1-PR4**: `DrawCardAction` の Apply 実装
   - 山札 → 現プレイヤー手札に 1 枚移動、TurnPhase 遷移
   - EARS / Gherkin、テスト
5. **M1-PR5**: `PlayCardAction` の Apply 実装
   - 手札 → Field に 1 枚移動、TurnPhase 遷移
   - EARS / Gherkin、テスト
6. **M1-PR6**: `EndTurnAction` の Apply 実装 + `ApplyActionUseCase`
   - TurnState.Next() を呼んでサブターン進行、TurnPhase 遷移
   - EARS / Gherkin、テスト
7. **M1-PR7**: 統合テスト(N=2 / 数ラウンド分の動作確認)
   - `StartGameUseCase` → `ApplyActionUseCase` の連鎖を end-to-end で確認
